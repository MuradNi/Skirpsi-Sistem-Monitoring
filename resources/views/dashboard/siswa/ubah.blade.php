...
@section('title', 'Edit Siswa — SD Perguruan Buddhi')
@section('page_title', 'Perbarui Profil Siswa')
...
<div class="max-w-2xl bg-white border border-gray-100 rounded-3xl p-8 shadow-sm">
  ...
  <div class="mb-6">
    ...
    <h3 class="font-extrabold text-gray-900 text-lg">Perbarui Informasi Siswa</h3>
  ...
  <form action="{{ route('dashboard.siswa.update', $siswa->id) }}" method="POST" class="space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
      <div>
        ...
        <input type="text" name="nis" required value="{{ old('nis', $siswa->nis) }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-primary">
      </div>
      <div>
        <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Nama Lengkap Siswa *</label>
        <input type="text" name="nama_lengkap" required value="{{ old('nama_lengkap', $siswa->nama_lengkap) }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-primary">
      </div>
    </div>
      ...
      <div>
        ...
        <select name="kelas_id" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-primary bg-white">
          @foreach($kelasList as $kelas)
            <option value="{{ $kelas->id }}" {{ old('kelas_id', $siswa->kelas_id) == $kelas->id ? 'selected' : '' }}>Kelas {{ $kelas->nama }}</option>
          @endforeach
        </select>
      </div>
      <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
      <div>
        ...
        <input type="date" name="tanggal_lahir" required value="{{ old('tanggal_lahir', $siswa->tanggal_lahir) }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-primary">
      </div>
      <div>
        <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">URL Foto Siswa (Opsional)</label>
        <input type="text" name="foto" value="{{ old('foto', $siswa->foto) }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-primary">
      </div>
    </div>
      ...
      <div>
        ...
        <select name="user_id" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-primary bg-white">
          ...
          @foreach($users as $user)
            <option value="{{ $user->id }}" {{ old('user_id', $siswa->user_id) == $user->id ? 'selected' : '' }}>{{ $user->nama }} ({{ $user->email }})</option>
          @endforeach
        </select>
      </div>
      <div>
        ...
        <select name="parent_user_id" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-primary bg-white">
          ...
          @foreach($parents as $parent)
            <option value="{{ $parent->id }}" {{ old('parent_user_id', $siswa->parent_user_id) == $parent->id ? 'selected' : '' }}>{{ $parent->nama }} ({{ $parent->email }})</option>
          @endforeach
        </select>
      </div>
    </div>
    ...
    <div class="flex items-center justify-end gap-3 pt-6">
      ...
      <button type="submit" class="px-5 py-2.5 bg-primary text-white text-xs font-bold rounded-xl hover:bg-primary-dark transition shadow-lg shadow-primary/20">Perbarui Profil</button>
    </div>
  </form>
</div>
@endsection
